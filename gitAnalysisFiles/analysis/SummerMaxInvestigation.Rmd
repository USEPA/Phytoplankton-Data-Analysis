...
#remove 1987 per Nate's email on 9/6/2016
BGtotals_summer <- mutate(filter(BGtotals_data, year!="1987"))
...
BGtotals_maxsummer <- BGtotals_summer  %>% 
                 ...
                 dplyr::summarise(
                        ...
                        datemax_cells_Ravg=date1[tot_cells_Ravg==max(tot_cells_Ravg)],
                        datemax_cells_Rmax=date1[tot_cells_Rmax==max(tot_cells_Rmax)],
                        num_samps_per_max=length(tot_cells_Rmax),
                        num_sampdates_per_max=length(unique(date1)))

 #                       datemax_bv_Ravg=date1[tot_bv_Ravg==max(tot_bv_Ravg)],
#                        datemax_bv_Rmax=date1[tot_bv_Rmax==max(tot_bv_Rmax)])
...
We need more investigation of when sampling is occurring and how much sampling is occuring with the maximum values we are reporting.   We will add investigations with temperature as soon as we get that data too.
...
```{r, eval=FALSE, include=FALSE}
plot(samp_dates, year) #just to look at relationship with dates over years, but really can do that easily with ggplot below -- maybe don't need all of above stuff


DatesPerYear_data <- data.frame()

#check
#with(filter(BGtotals_data, lake=="EFR", year=="1992", depth_cat=="0-3 ft"), unique(yday(date1)))

## Only for summer data (summer max)
```
