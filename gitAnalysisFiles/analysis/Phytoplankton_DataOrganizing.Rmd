---
title: "Data Organization - makes files for further analysis"
date: "updated September 14, 2016"
output:
  html_document:
    toc: true
    theme: united
...
```{r FixDate}
#Make it so R recognizes date as a date
...
alg2 <- mutate(filter(alg, station %in% good_stations), lake_station=paste(lake,station), 
                 lake_station_date=paste(lake,station,date), lake_station_date_depth=paste(lake,station,date,depth_ft))
...
```{r}
#Including hab samples

with(filter(alg2, hab==1), length(unique(lake_station_date))) #819 hab

#Excluding hab samples
alg3 <- mutate(filter(alg, station %in% good_stations, hab==0), BV.um3.L=as.numeric(BV.um3.L),
...
The qual_replicate column has Q's and R's.  For this analysis, we removed all observations with "Q's" and for field replicates, we calculate the average of the two replicate samples, as well as pulling the maximum of the two.  Both are available for analysis and can be compared later.


```{r InvestigateDuplicates1, eval=FALSE, include=FALSE}
#WHY ARE THERE NO "R"'s adnd "Q"'s now?
with(alg3, unique(qual_replicate)) #NA, "R", "Q"
...
#To look specifically at a date with "Q"'s and "R"'s
#It looks like Q's are actually repeat values already included as R's, so I am going to remove
select(arrange(filter(alg3, date=="20070911", depth_ft==0), date1, station), date1, station, lake, taxa, depth_ft, cell_per_l, qual_replicate)
...
#with(alg3, sum(is.na(qual_replicate)==TRUE))
...
alg3_NAonly <- dplyr::select(mutate(filter(alg3_noQ, is.na(qual_replicate)==TRUE), 
                      ...
                      lake_station, depth_ft, date1, taxa, cells_Rmax, bv_Rmax, cells_Ravg, bv_Ravg,
                      toxin, sheet_id, Taxa.Type, lake, station)
...
# for each station/lake.  This does account for multiple depths because different depths are
...
NOT SURE I NEED THESE -- MIGHT BE CALCULATING THESE WITHIN THE TOTALS CODE
...
# Select division for the analysis

The different groupings we will look at are Diatoms, Green Algae, Toxin producers, total phytoplankton

## Cyanobacteria - Blue-Green Algae

```{r}
#NOT SURE I NEED THIS CODE CHUNCK
...
Should use genus to do this and it is just exploratory.  We are not planning to use this a response variable, but it would be nice to know how many taxa are going into each count and possible include that information.  Interesting to look at the relationship between cell counts and/or biovolume and number of taxa identified in a sample.
...
#NOT SURE I NEED THIS, BUT SHOULD LOOK AT THIS BECAUSE COULD CATCH SOME REMAINING HAB SAMPLES
BGnum_taxa_data <- alg_bg %>% group_by(lake, station, date1, depth_cat) %>% dplyr::summarise(num_taxa=n())
dim(BGnum_taxa_data) #10697 x 5
...
#Method 1: This EXCLUDES all stations with NO Blue-Green Algae entries in the Taxa.Type column
# Therefore, we're possibly excluding zeroes that should be included
alg_bg <- filter(alg3_final, Taxa.Type=="Blue-Green Algae")
BGtotals_no0s_data <- alg_bg %>% 
                 group_by(lake, station, date1, depth_cat) %>%
                 dplyr::summarise(tot_cells_Rmax=sum(cells_Rmax), tot_bv_Rmax=sum(bv_Rmax),
...
Need to also create response variable that is the maximum from June through September for each year
...
# This does not work for some lakes if try to do it by station
...
```{r, eval=FALSE, include=FALSE}
#Go through example of one that's not working above
#Can do it by lake if want to in analysis code b/c it will work for some
test <- filter(BGtotals_summer, lake %in% c("RRR")) %>%
            dplyr::group_by(lake, year) %>%
            dplyr::summarise(datemax=date1[tot_cells_Ravg==max(tot_cells_Ravg)]


test_dat <- filter(BGtotals_summer, lake=="RRR", station=="20001", depth_cat=="0-3 ft")
View(test_dat)
with(test_dat, date1[tot_cells_Ravg==max(tot_cells_Ravg)])

BGtotals_summer$year[1:100]
```
...
### Total cells per liter - Blue Green Algae

Total cell density is calculated as the sum of the cells/L measurements for all taxa in the grouping (e.g. Blue-Green Algae) found in a sample.  Therefore, some of these counts have more taxa included in them than others (see previous plots).  
...
### Total biovolume (um3/L) - Blue Green Algae

Total biovolume is calculated as the sum of the biovolume measurements ($\mu m ^3/L$) for all Blue-Green Algae found in a sample.  Therefore, some of these counts have more taxa included in them than others (see previous plots). 
...
## Total cells/L and biovolume over all taxa

NEED TO THINK MORE ABOUT THE DIFFERENCES BETWEEN THE TWO ALL TAXA SUMS -- WHY DOES THE NA.OMIT GIVE LESS O'S THAN NA'S THAT ARE PICKED UP IN FIRST ONE?

These totals make up the denominator for the relative (i.e. dominance) measures, but we also calculate them separately so they can be analyzed on their own later.
```{r totalcellsALL, include=FALSE}
...
with(BGdom_data, sum(is.na(rel_cells_Ravg)==TRUE)) #0 - without the na.omit above there are over 9000 NA's for any 
...
```{r BGdominanceNONA, include=FALSE}
...
## USED THIS TO JUST CHECK SUMMARISE() 
...
toxin_totals_maxsummer <- toxin_totals_summer  %>% 
   ...
   #Issue --- when there are only 0's for a particular year-lake combination and there are multiple occasions
   # with zeroes - then there are multiple dates for the max, which messes with this.  I'm just going to choose
   # the maximum date when this is the case.  I feel like I tried to grab the first one before and it didn't work.
