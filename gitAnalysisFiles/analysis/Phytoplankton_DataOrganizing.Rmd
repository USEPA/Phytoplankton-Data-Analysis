...
UPDATE: For new data set as of 9/13/16, there are no 'R's or Q's included. Will is looking into this.  Also look at the Wiki for how Matt created the 'R's and Q's to begin with.  If they were just for HAB samples, then we might not need the rest of this code.
...
#This code chunk just checks things and does not need to be run
...
Make a new depth column called depth_cat
...
## Create BG total cell density and total biovolume for every unique sample
...
#NOTE:  NEED TO THINK MORE ABOUT THE DIFFERENCES BETWEEN THE TWO ALL TAXA SUMS -- WHY DOES THE NA.OMIT GIVE LESS O'S THAN NA'S THAT ARE PICKED UP IN FIRST ONE?
...
toxin_totals_maxsummer <- toxin_totals_summer  %>% 
   ...
   # the maximum date when this is the case. 
...
I do think we should be checking the number of taxa going into each count and make sure
they are approximately where we would expect them.
...
# Plots used in originally getting to know the data
